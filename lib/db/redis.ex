defmodule Db.Redis do
  ...
  def get_binary(key) do
    GenServer.call(Db.Redis, {:get_binary, key})
  end


  def reset() do
    GenServer.cast(Db.Redis, :reset)
  end

  def delete(key) do
    GenServer.cast(Db.Redis, {:delete, key})
  end

  def set_binary(key, value) do
    GenServer.cast(Db.Redis, {:set_binary, key, value})
  end

  def set_map(key, value) do
    GenServer.cast(Db.Redis, {:set_map, key, value})
  end


  def get_map(key, struct \\ nil) do
    GenServer.call(Db.Redis, {:get_map, key, struct})
  end

 
  def push(key, value) do
    GenServer.cast(Db.Redis, {:push, key, value})
  end

  def pop(key) do
    GenServer.call(Db.Redis, {:pop, key})
  end


  def get_list(key) do
    GenServer.call(Db.Redis, {:get_list, key})
  end
  ...
  def handle_cast({:set_map, key, value}, redis) do
    value = if Map.has_key?(value, :__struct__) do
      Map.from_struct(value)
    else
      value
    end

    keys_and_values  = Enum.flat_map(value, fn {k, v} ->
      if is_number(v) do
