...
library RSA {
    bytes1 public constant ONE = 0x01;
    ...
    */
    function pkcs1Sha256Verify(bytes32 _sha256, bytes memory _s, bytes memory _e, bytes memory _m) public view returns (uint) {
        ...
        if (decipher[0] != 0 || decipher[1] != ONE) {
            return 1;
        }
    ...
    /** @dev Verifies a PKCSv1.5 SHA256 signature
      * @param _data is the message
      * @param _s is the signature
      * @param _e is the exponent
      * @param _m is the modulus
      * @return 0 if success, >0 otherwise
    */
    function pkcs1Sha256VerifyRaw(bytes memory _data, bytes memory _s, bytes memory _e, bytes memory _m) public view returns (uint) {
