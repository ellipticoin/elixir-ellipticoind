...
func createPubSub(secretKey []byte, port int, bootnodes []string) (*libp2pPubSub, *core.Host) {
	pubsub := new(libp2pPubSub)
	host := pubsub.createPeer(secretKey, port)
...
func main() {
	...
	// hostname, _ := flag.Args()[1]
	port, _ := strconv.Atoi(flag.Args()[2])
	bootnodes := flag.Args()[3:]

	pubsub, host := createPubSub(secretKey, port, bootnodes)
