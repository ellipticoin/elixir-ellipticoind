...
func (c *libp2pPubSub) createPeer(secretKey []byte, port int) *core.Host {
	// Creating a node
	h, err := createHost(secretKey, port)
	if err != nil {
		panic(err)
	}
...
func createHost(secretKey []byte, port int) (core.Host, error) {
	sk, err := crypto.UnmarshalSecp256k1PrivateKey(secretKey)
