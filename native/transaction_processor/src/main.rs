...
fn main() {
    ...
    loop {
        let transaction_bytes: Vec<u8> = conn.brpoplpush("transactions::queued", "transactions::processing", 0).unwrap();
        ...
        println!("done: {:?}", transaction_result);
        let _: () = vm::pipe()
            .atomic()
            .cmd("LPOP").arg("transactions::processing").ignore()
            .cmd("RPUSH").arg("transactions::done").arg(transaction_result).ignore().query(&conn).unwrap();
