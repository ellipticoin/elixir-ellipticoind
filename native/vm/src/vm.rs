...
impl<'a> VM<'a> {
    pub fn new(
        ...
        externals: &'a mut EllipticoinExternals<'a>,
    ) -> VM<'a> {
        VM {
                instance,
                externals,
    ...
    pub fn write_pointer(&mut self, vec: Vec<u8>) -> Result<Option<RuntimeValue>, metered_wasmi::Trap> {
    ...
    pub fn namespaced_key(&mut self, key: Vec<u8>) -> Vec<u8> {
        ...
        let mut contract_name = self.externals.transaction.contract_name.as_bytes().to_vec().clone();
    ...
    pub fn call(
        ...
        match self.instance.invoke_export(func, &runtime_values, self) {
            Ok(Some(RuntimeValue::I32(value))) => {
                ...
                (result::from_bytes(self.read_pointer(value)), self.externals.gas)
            }
    ...
    pub fn read_memory(&self, pointer: i32, length: i32) -> Vec<u8> {
        match self.instance.export_by_name("memory").expect("error reading memory") {
