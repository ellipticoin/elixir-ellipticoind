...
impl From<BTreeMap<String, Value>> for Transaction {
    fn from(transaction: BTreeMap<String, Value>) -> Self {
        let params = transaction.get("params").unwrap().as_array().unwrap().to_vec();
        let mut env: HashMap<String, Vec<u8>> = HashMap::new();
        let map = transaction.get("env").unwrap().as_object().unwrap();
        for (key, value) in map {
            env.insert(key.as_string().unwrap().to_string(), to_vec(value).unwrap());
        }

        Transaction {
            code: transaction.get("code").unwrap().as_bytes().unwrap().to_vec(),
            method: transaction.get("method").unwrap().as_string().unwrap().to_string(),
            env: env,
            params: params,
        }
    }
...
pub fn transaction_from_slice(transaction_bytes: &[u8]) -> Transaction {
    from_slice::<BTreeMap<String, Value>>(&transaction_bytes).unwrap().into()
}
...
pub fn run_transaction(transaction: &Transaction, db: &Connection) -> Vec<u8>{
    ...
    let params: Vec<RuntimeValue> = transaction.params.iter().map(|param| {
